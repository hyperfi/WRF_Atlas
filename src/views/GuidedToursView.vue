  ...
  <div class="guide-view">
    ...
    <section v-else class="build-layout">
      <div class="build-intro surface-panel">
        ...
        <p>Start with the Ubuntu / GNU Fortran idealized example, or inspect the checkout's other build paths. These commands build WRF on your machine; the Atlas does not compile or run it for you.</p>
      </div>
      <div class="build-paths">
        <article v-for="path in buildPaths" :key="path.id" class="build-path surface-panel" :class="{ selected: selectedBuildPath === path.id }">
          ...
          <template v-if="selectedBuildPath === path.id">
            <p v-if="path.note" class="build-note">{{ path.note }}</p>
            <ol><li v-for="step in path.steps" :key="step.command"><div><strong>{{ step.title }}</strong><p>{{ step.detail }}</p><dl v-if="step.prompts" class="configure-prompts"><div v-for="prompt in step.prompts" :key="prompt.question"><dt>{{ prompt.question }}</dt><dd>{{ prompt.answer }}</dd></div></dl></div><code>{{ step.command }}</code></li></ol>
      ...
      <div class="prerequisite-panel surface-panel">
        ...
        <p>The Ubuntu example is a serial, distro-package starting point, not a universal compiler recipe. Use libraries built for the same toolchain; MPI and other configurations need their own checks.</p>
      </div>
    </section>
  </div>
</template>
...
const buildPaths = [
  { id: 'ubuntu-gnu', code: 'GNU', title: 'Ubuntu + gfortran: idealized supercell', subtitle: 'Serial em_quarter_ss with NetCDF installed under /usr.', evidence: { path: 'README', line: 324 }, note: 'Run these commands in a Bash shell from the root of a WRF source checkout, except after the cd step. No WPS, real.exe, or external meteorological input is needed for this idealized case. Package names are for Ubuntu; verify them for your release.', steps: [
    { title: 'Install the serial build dependencies', detail: 'Ubuntu provides the C and Fortran NetCDF development interfaces separately. The build tools and csh support the classic WRF scripts.', command: 'sudo apt update && sudo apt install build-essential gfortran csh m4 perl libnetcdf-dev libnetcdff-dev' },
    { title: 'Verify the /usr installation', detail: 'Both config tools should exist, and both prefixes should report /usr. If either is missing or points elsewhere, resolve that before configuring.', command: 'gfortran --version; nc-config --prefix; nf-config --prefix' },
    { title: 'Select NetCDF in /usr', detail: 'The checkout requires NETCDF to be set. On Ubuntu, actual libraries may be in /usr/lib/<architecture>-linux-gnu; nf-config supplies their linker flags.', command: 'export NETCDF=/usr' },
    { title: 'Answer the configure prompts', detail: 'Read the menu labels rather than copying a fixed selection number; the numbering can change with WRF version and platform. For this one-domain idealized example, choose no nesting.', command: './configure', prompts: [
      { question: 'Enter selection [1–N]', answer: 'Type the number on the line labeled GNU (gfortran/gcc) and (serial). Do not choose smpar or dmpar for this example.' },
      { question: 'Compile for nesting? [default 0]', answer: 'Type 0 and press Enter (or press Enter to accept the serial default of 0: no nesting).' },
    ] },
    { title: 'Check what was selected', detail: 'The generated configuration should identify gfortran, gcc, and nesting option 0. If not, rerun configure and choose the matching menu entry.', command: "grep -E '^(SFC|SCC)[[:space:]]*=' configure.wrf; grep 'Nesting option:' configure.wrf" },
    { title: 'Compile the idealized case', detail: 'The local WRF README lists em_quarter_ss. Capture both standard output and errors in the build log.', command: './compile em_quarter_ss > compile.log 2>&1' },
    { title: 'Confirm the two executables', detail: 'A successful ideal-case build produces ideal.exe and wrf.exe; the test directory contains links to them.', command: 'test -x main/ideal.exe && test -x main/wrf.exe && tail -n 20 compile.log' },
    { title: 'Generate the initial state', detail: 'Use the case-provided namelist.input and input_sounding. ideal.exe should create wrfinput_d01.', command: 'cd test/em_quarter_ss && ./ideal.exe > ideal.log 2>&1 && test -s wrfinput_d01' },
    { title: 'Run WRF and inspect the result', detail: 'Run the idealized model, then check the run log and output files. This is a smoke test, not scientific validation.', command: './wrf.exe > wrf.log 2>&1 && tail -n 20 wrf.log && ls wrfout_d01_*' },
  ]},
...
const selectedBuildPath = ref('ubuntu-gnu')
...
.build-paths { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }.build-path { min-width: 0; overflow: hidden; }.build-path.selected { grid-column: 1 / -1; }.build-path-header { display: grid; grid-template-columns: 44px 1fr 66px; align-items: center; gap: 12px; padding: 18px; }.build-path-header > span { color: var(--accent-emerald); font-family: var(--font-mono); font-size: .6rem; }.build-path-header h3 { font-size: .84rem; }.build-path-header p { margin-top: 2px; color: var(--text-muted); font-size: .62rem; }.build-path-header button { padding: 6px; background: var(--bg-inset); border: 1px solid var(--border-subtle); border-radius: 4px; color: var(--text-secondary); cursor: pointer; font-size: .59rem; }.build-path.selected .build-path-header { background: var(--accent-soft); }.build-note { max-width: 900px; padding: 12px 18px; color: var(--text-secondary); font-size: .64rem; line-height: 1.55; }.build-path ol { border-top: 1px solid var(--border-subtle); list-style: none; }.build-path li { display: grid; grid-template-columns: minmax(190px,1fr) minmax(280px,1.1fr); gap: 15px; padding: 13px 18px; border-bottom: 1px solid var(--border-subtle); }.build-path li strong { font-size: .68rem; }.build-path li p { margin-top: 3px; color: var(--text-muted); font-size: .59rem; }.build-path li > code { align-self: center; overflow-x: auto; padding: 7px; background: var(--bg-inset); color: var(--accent-amber); font-size: .57rem; white-space: nowrap; }.build-proof { display: grid; grid-template-columns: 80px 1fr auto; align-items: center; gap: 8px; padding: 12px 18px; font-size: .58rem; }.build-proof span { color: var(--text-muted); }.build-proof code { color: var(--text-secondary); }.build-proof button { padding: 5px 7px; background: transparent; border: 1px solid var(--border-subtle); border-radius: 4px; color: var(--text-secondary); cursor: pointer; font-size: .57rem; }.prerequisite-panel { display: grid; grid-template-columns: 130px repeat(3,1fr); gap: 20px; padding: 18px 20px; }.prerequisite-panel > div { display: flex; flex-direction: column; gap: 4px; }.prerequisite-panel strong { font-size: .68rem; }.prerequisite-panel span,.prerequisite-panel > p:last-child { color: var(--text-muted); font-size: .59rem; line-height: 1.45; }.prerequisite-panel > p:last-child { grid-column: 2 / -1; }
.configure-prompts { display: grid; gap: 8px; margin-top: 12px; }.configure-prompts > div { padding: 9px 11px; border-left: 2px solid var(--accent-emerald); background: var(--bg-inset); }.configure-prompts dt { color: var(--text-secondary); font-family: var(--font-mono); font-size: .58rem; }.configure-prompts dd { margin-top: 4px; color: var(--text-muted); font-size: .59rem; line-height: 1.45; }
...
@media(max-width:850px){.page-header{align-items:flex-start;flex-direction:column}.guide-tabs{grid-template-columns:1fr 1fr}.learn-layout,.input-layout,.research-layout{grid-template-columns:1fr}.lesson-intro,.research-hero{flex-direction:column}.workflow-notes{grid-template-columns:1fr}.workflow-caution{grid-template-columns:1fr}.workflow-intro,.build-intro{grid-template-columns:1fr}.workflow-stage{grid-template-columns:30px 1fr}.stage-io{grid-column:2}.build-path li{grid-template-columns:1fr}.prerequisite-panel{grid-template-columns:1fr}.prerequisite-panel>p:last-child{grid-column:1}.source-stop{grid-template-columns:28px 1fr}.source-stop>button{grid-column:2;width:max-content}}
</style>
